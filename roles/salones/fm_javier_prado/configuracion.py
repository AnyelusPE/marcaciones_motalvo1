...
def run_configuracion():
    ...
    st.markdown("Esta sección está diseñada para administrar las preferencias del salón **FM Javier Prado**.")
    st.divider()

    # Sección general de información
    with st.expander("🏷️ Información del salón", expanded=True):
        st.text_input("Nombre del salón", "FM Javier Prado", disabled=True)
        st.text_input("Ubicación", "Javier Prado, Magdalena", disabled=True)
