...
def run_subir_horarios():
    ...
    carpeta = os.path.join("data", "uploads", "trujillo_mall")
    os.makedirs(carpeta, exist_ok=True)
    nombre_archivo = f"TRUJILLO_MALL_{año}-{mes:02d}-{q}.csv"
    ...
    edited_df = st.data_editor(
        ...
        key="editor_trujillo_mall"
    )

    if st.button("💾 Guardar cambios", key="guardar_trujillo_mall"):
