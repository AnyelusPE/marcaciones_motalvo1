...
def run_configuracion():
    ...
    st.markdown("Esta sección está diseñada para administrar las preferencias del salón **Trujillo Mall**.")
    st.divider()

    # Sección general de información
    with st.expander("🏷️ Información del salón", expanded=True):
        st.text_input("Nombre del salón", "Trujillo Mall", disabled=True)
        st.text_input("Ubicación", "Trujillo Mall, Trujillo", disabled=True)
