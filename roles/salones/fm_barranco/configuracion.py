...
def run_configuracion():
    ...
    st.markdown("Esta sección está diseñada para administrar las preferencias del salón **FM Barranco**.")
    st.divider()

    # Sección general de información
    with st.expander("🏷️ Información del salón", expanded=True):
        st.text_input("Nombre del salón", "FM Barranco", disabled=True)
        st.text_input("Ubicación", "Barranco, Lima", disabled=True)
